...
# Rig dashboard integration. These values are deliberately server-side only.
# The dashboard never receives the Prometheus URL or any monitoring token.
# Access is disabled unless at least one explicit allow-list setting exists.
RIG_MONITORING_API_URL = os.environ.get("RIG_MONITORING_API_URL", "").strip().rstrip("/")
RIG_MONITORING_API_TOKEN = os.environ.get("RIG_MONITORING_API_TOKEN", "").strip()
RIG_INGEST_TOKEN = os.environ.get("RIG_INGEST_TOKEN", "").strip()
RIG_DASHBOARD_CLIENT_TOKEN = os.environ.get("RIG_DASHBOARD_CLIENT_TOKEN", "").strip()
RIG_DASHBOARD_TENANTS = {
    s.strip() for s in os.environ.get("RIG_DASHBOARD_TENANTS", "").split(",") if s.strip()
}
RIG_GRAFANA_URL = os.environ.get("RIG_GRAFANA_URL", "").strip()
RIG_BMC_URL = os.environ.get("RIG_BMC_URL", "").strip().rstrip("/")
RIG_BMC_USERNAME = os.environ.get("RIG_BMC_USERNAME", "").strip()
RIG_BMC_PASSWORD = os.environ.get("RIG_BMC_PASSWORD", "")
RIG_BMC_SYSTEM_PATH = os.environ.get("RIG_BMC_SYSTEM_PATH", "/redfish/v1/Systems/1").strip()
RIG_POWER_CONTROL_ENABLED = os.environ.get("RIG_POWER_CONTROL_ENABLED", "false").lower() in {"1", "true", "yes"}
RIG_BMC_CA_BUNDLE = os.environ.get("RIG_BMC_CA_BUNDLE", "").strip()
RIG_LATEST_SNAPSHOT = None
...
async def client_me(token: str = Query("")):
    ...
    return {
        ...
        "assistant_ids": ids,
        "rig_access": _rig_client_has_access(token, c),
...
# --- Private rig operator view ------------------------------------------------
# This is intentionally separate from the tenant data path above. Prometheus
# stays on the rig; the browser receives only this small, normalised payload.

RIG_QUERY_EXPRESSIONS = {
    "gpu_core_temperature_celsius": "DCGM_FI_DEV_GPU_TEMP",
    "gpu_power_watts": "DCGM_FI_DEV_POWER_USAGE",
    "gpu_utilization_percent": "DCGM_FI_DEV_GPU_UTIL",
    "gpu_memory_used_bytes": "DCGM_FI_DEV_FB_USED",
    "gpu_memory_free_bytes": "DCGM_FI_DEV_FB_FREE",
    "gpu_vram_temperature_celsius": "rig_gpu_vram_temperature_celsius",
    "bmc_temperature_celsius": "ipmi_temperature_celsius",
    "bmc_temperature_threshold_celsius": "ipmi_temperature_threshold_celsius",
    "fan_speed_rpm": 'ipmi_fan_speed_rpm{name="FAN1"}',
    "smart_status": "smartctl_device_smart_status",
    ...
    "collector_up": 'up{job=~"node|dcgm|ipmi|smartctl"}',
    "vllm_up": 'up{job=~"vllm|vllm-qwen36"}',
    "storage_free_bytes": 'node_filesystem_avail_bytes{mountpoint="/mnt/data"}',
    "active_alerts": 'ALERTS{alertstate="firing"}',
}


def _rig_client_has_access(token: str, client_row: dict) -> bool:
    """Return true only for the explicitly configured operator client."""
    token_match = bool(
        RIG_DASHBOARD_CLIENT_TOKEN
        and hmac.compare_digest(token, RIG_DASHBOARD_CLIENT_TOKEN)
    )
    tenant_match = bool(client_row.get("tenant_slug") in RIG_DASHBOARD_TENANTS)
    return token_match or tenant_match


def _rig_api_url(path: str) -> str:
    base = RIG_MONITORING_API_URL.rstrip("/")
    if base.endswith("/api/v1"):
        return f"{base}/{path.lstrip('/')}"
    return f"{base}/api/v1/{path.lstrip('/')}"


def _rig_samples(document: dict) -> list[dict]:
...
async def _rig_prom_query(client: httpx.AsyncClient, expression: str) -> list[dict]:
    response = await client.get(
        _rig_api_url("query"),
        params={"query": expression},
        headers={"Authorization": f"Bearer {RIG_MONITORING_API_TOKEN}"}
        if RIG_MONITORING_API_TOKEN else {},
    )
    response.raise_for_status()
    document = response.json()
    if document.get("status") != "success":
        raise RuntimeError("Prometheus query failed")
    return _rig_samples(document)


def _rig_gpu_id(labels: dict, fallback: str) -> str:
    for key in ("gpu", "GPU", "index", "device"):
        if labels.get(key) is not None:
            return str(labels[key])
    return fallback


def _rig_metric_value(samples: list[dict], label_name: str = "") -> float | None:
    if label_name:
        samples = [s for s in samples if s["labels"].get("name") == label_name]
    return samples[0]["value"] if samples else None


def _rig_store_snapshot(snapshot: dict) -> None:
    """Cache the latest credential-free rig snapshot in the app database."""
    global RIG_LATEST_SNAPSHOT
    RIG_LATEST_SNAPSHOT = snapshot
    encoded = json.dumps(snapshot, separators=(",", ":"), sort_keys=True)
    now = datetime.utcnow().isoformat(timespec="seconds") + "Z"
    try:
        with get_db() as conn:
            conn.execute(
                "CREATE TABLE IF NOT EXISTS rig_health_latest ("
                "id INTEGER PRIMARY KEY, snapshot_json TEXT NOT NULL, received_at TEXT NOT NULL)"
            )
            conn.execute(
                "INSERT INTO rig_health_latest (id, snapshot_json, received_at) VALUES (1, ?, ?) "
                "ON CONFLICT (id) DO UPDATE SET snapshot_json = EXCLUDED.snapshot_json, received_at = EXCLUDED.received_at",
                (encoded, now),
            )
            conn.commit()
    except Exception as error:
        # The in-memory copy still serves the current process. Log only the
        # database error; never log the snapshot body or ingest token.
        print(f"[rig-ingest] database cache failed: {type(error).__name__}", file=sys.stderr)


def _rig_load_snapshot() -> dict | None:
    if isinstance(RIG_LATEST_SNAPSHOT, dict):
        return RIG_LATEST_SNAPSHOT
    try:
        with get_db() as conn:
            row = conn.execute(
                "SELECT snapshot_json FROM rig_health_latest WHERE id = 1"
            ).fetchone()
...
@app.post("/api/rig/ingest")
async def rig_ingest(request: Request):
    """Accept a credential-free health snapshot from the rig over HTTPS."""
    if not RIG_INGEST_TOKEN:
        raise HTTPException(status_code=503, detail="rig_ingest_not_configured")
    presented = request.headers.get("X-Rig-Ingest-Token", "")
    if not presented or not hmac.compare_digest(presented, RIG_INGEST_TOKEN):
    ...
    if not isinstance(snapshot, dict) or not isinstance(snapshot.get("metrics"), dict):
        raise HTTPException(status_code=422, detail="snapshot_metrics_required")
    if len(json.dumps(snapshot, separators=(",", ":"))) > 1_000_000:
        raise HTTPException(status_code=413, detail="snapshot_too_large")
    _rig_store_snapshot(snapshot)
    return {"ok": True, "received_at": datetime.utcnow().isoformat(timespec="seconds") + "Z"}


@app.get("/client/api/rig/status")
async def client_rig_status(token: str = Query("")):
    """Return a small operator-facing summary from the rig Prometheus API."""
    c = check_client(token)
    if not _rig_client_has_access(token, c):
        # Do not confirm that the rig dashboard exists to other tenants.
        raise HTTPException(status_code=404, detail="not_found")
    if not RIG_MONITORING_API_URL:
        snapshot = _rig_load_snapshot()
        if not snapshot:
            raise HTTPException(status_code=503, detail="rig_monitoring_not_configured")
        snapshot_ts = snapshot.get("ts")
        sample_ts = None
        if snapshot_ts:
            try:
                sample_ts = datetime.fromisoformat(str(snapshot_ts).replace("Z", "+00:00")).timestamp()
            except ValueError:
                pass
        return {
            "mode": "snapshot",
            "status": "degraded" if snapshot.get("status") == "partial" else "operational",
            "updated_at": snapshot_ts,
            "sample_ts": sample_ts,
            "errors": [item.get("metric", "unknown") for item in (snapshot.get("errors") or [])],
            "alerts": [],
            "snapshot": snapshot,
            "links": {"grafana": RIG_GRAFANA_URL or None},
            "power": {"enabled": bool(RIG_POWER_CONTROL_ENABLED and RIG_BMC_URL and RIG_BMC_USERNAME and RIG_BMC_PASSWORD)},
            "history": {"snapshot_interval": "5m", "retention": "365d Prometheus"},
        }
    ...
    async with httpx.AsyncClient(timeout=8) as client:
        jobs = {
            name: asyncio.create_task(_rig_prom_query(client, expression))
            for name, expression in RIG_QUERY_EXPRESSIONS.items()
        }
        for name, job in jobs.items():
            try:
                queries[name] = await job
            except Exception:
                queries[name] = []
                errors.append(name)
    ...
    gpu_maps: dict[str, dict] = {}
    metric_fields = {
        "gpu_core_temperature_celsius": "core_temp_celsius",
        "gpu_vram_temperature_celsius": "vram_temp_celsius",
        "gpu_utilization_percent": "utilization_percent",
        "gpu_power_watts": "power_watts",
        "gpu_memory_used_bytes": "memory_used_bytes",
        "gpu_memory_free_bytes": "memory_free_bytes",
    }
    for metric_name, field in metric_fields.items():
        for index, sample in enumerate(queries.get(metric_name, [])):
            labels = sample["labels"]
            gpu_id = _rig_gpu_id(labels, str(index))
            gpu = gpu_maps.setdefault(gpu_id, {"id": gpu_id, "name": ""})
            gpu[field] = sample["value"]
            gpu["name"] = (
                labels.get("modelName") or labels.get("model_name")
                or labels.get("product_name") or gpu.get("name") or ""
            )
    gpus = [gpu_maps[key] for key in sorted(gpu_maps, key=lambda value: (len(value), value))]

    threshold_map = {}
    for sample in queries.get("bmc_temperature_threshold_celsius", []):
        labels = sample["labels"]
        name = labels.get("name", "")
        threshold = labels.get("threshold", "")
        threshold_map.setdefault(name, {})[threshold] = sample["value"]
    bmc = []
    for sample in queries.get("bmc_temperature_celsius", []):
        name = sample["labels"].get("name", "Sensor")
        bmc.append({
            "name": name,
            "value_celsius": sample["value"],
            "upper_noncritical_celsius": threshold_map.get(name, {}).get("upper_noncritical"),
            "upper_critical_celsius": threshold_map.get(name, {}).get("upper_critical"),
        })

    def latest_sample(name: str) -> float | None:
        samples = queries.get(name, [])
        return max((s["ts"] for s in samples if s.get("ts") is not None), default=None)

    sample_times = [latest_sample(name) for name in queries]
    sample_times = [stamp for stamp in sample_times if stamp is not None]
    sample_ts = max(sample_times, default=None)
    collectors = queries.get("collector_up", [])
    collector_healthy = sum(1 for sample in collectors if sample["value"] == 1)
    collector_total = len(collectors)
    vllm_samples = queries.get("vllm_up", [])
    vllm_status = "running" if any(sample["value"] == 1 for sample in vllm_samples) else (
        "not_instrumented" if not vllm_samples else "down"
    )
    fan_rpm = _rig_metric_value(queries.get("fan_speed_rpm", []), "FAN1")
    memory_total = _rig_metric_value(queries.get("node_memory_total_bytes", []))
    memory_available = _rig_metric_value(queries.get("node_memory_available_bytes", []))
    storage_free = _rig_metric_value(queries.get("storage_free_bytes", []))
    smart_samples = queries.get("smart_status", [])
    smart_healthy = sum(1 for sample in smart_samples if sample["value"] == 1)
    status = "degraded" if firing or errors else "operational"
    if not gpus and not collectors:
        status = "unavailable"
    ...
    return {
        ...
        "alerts": firing,
        "gpus": gpus,
        "fan": {"name": "FAN1", "rpm": fan_rpm, "bank_note": "Chained PWM bank; FAN1 is the visible tachometer."},
        "host": {
            "load1": _rig_metric_value(queries.get("node_load1", [])),
        ...
        "services": {
            ...
            "collectors_total": collector_total,
        },
        "smart": {"healthy": smart_healthy, "total": len(smart_samples)},
        "bmc_temperatures": sorted(bmc, key=lambda row: row["value_celsius"], reverse=True),
        "links": {"grafana": RIG_GRAFANA_URL or None},
        "power": {"enabled": bool(RIG_POWER_CONTROL_ENABLED and RIG_BMC_URL and RIG_BMC_USERNAME and RIG_BMC_PASSWORD)},
        "history": {"snapshot_interval": "5m", "retention": "365d Prometheus"},
    }
...
@app.post("/client/api/rig/power")
async def client_rig_power(request: Request, token: str = Query("")):
    """Perform an explicitly enabled Redfish power action for the rig."""
    c = check_client(token)
    if not _rig_client_has_access(token, c):
        raise HTTPException(status_code=404, detail="not_found")
    if not RIG_POWER_CONTROL_ENABLED:
        raise HTTPException(status_code=503, detail="rig_power_control_disabled")
    if not all([RIG_BMC_URL, RIG_BMC_USERNAME, RIG_BMC_PASSWORD]):
        raise HTTPException(status_code=503, detail="rig_bmc_control_not_configured")
    try:
        body = await request.json()
    except Exception:
        raise HTTPException(status_code=400, detail="invalid_json")
    ...
    if action not in {"on", "off"}:
        raise HTTPException(status_code=422, detail="action_must_be_on_or_off")
    if action == "off" and body.get("confirmation") != "POWER DOWN EPYC RIG":
        raise HTTPException(status_code=422, detail="power_down_confirmation_required")

    verify = RIG_BMC_CA_BUNDLE or False
    auth = (RIG_BMC_USERNAME, RIG_BMC_PASSWORD)
    system_url = f"{RIG_BMC_URL}/{RIG_BMC_SYSTEM_PATH.lstrip('/')}"
    try:
        async with httpx.AsyncClient(timeout=15, verify=verify, follow_redirects=True, auth=auth) as client:
            system_response = await client.get(system_url, headers={"Accept": "application/json"})
            ...
            target = reset_action.get("target") or f"{system_url}/Actions/ComputerSystem.Reset"
            if target.startswith("/"):
                from urllib.parse import urljoin
                target = urljoin(RIG_BMC_URL + "/", target)
            reset_type = "On" if action == "on" else "GracefulShutdown"
        ...
        raise HTTPException(status_code=502, detail="bmc_power_action_failed")
    except (httpx.HTTPError, ValueError, KeyError):
        raise HTTPException(status_code=502, detail="bmc_power_connection_failed")

    return {"ok": True, "action": action, "reset_type": reset_type, "requested_at": datetime.utcnow().isoformat(timespec="seconds") + "Z"}
...
# Reuses check_admin(token) defined earlier in this file. PB1 default:
# customer's token IS their admin_token (no new auth code).
#
...
# env. 503 cleanly when not wired.
#
# Source-of-truth: operator MVP alembic 0010 (request_queue +
# tier_sla_hours + time_entry, document-ops-portal). PB3 default applied:
# ship after operator MVP, which is live.
...
def _portal_db_connect():
    ...
    container `m19o67kvb3d2ugvobm9zs9z4`).
    """
    ...
    if not url:
        raise HTTPException(
            ...
            detail="PORTAL_PG_URL env var not set. "
                   "Wire the document-ops-portal Postgres DSN in Render "
